...
test("paired webhook streams Telegram progress updates and sends the final answer separately", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "opencolab-chat-progress-stream-"));
  const sentTexts: string[] = [];
  const progressMessages: string[] = [];
  const progressEdits: Array<{ messageId: number; text: string }> = [];
  let typingCalls = 0;
  ...
  const runtime = createRuntime(tempDir, {
    ...
    telegramProgressSender: async (_chatId, text) => {
      progressMessages.push(text);
      return { ok: true, messageId: 77 };
    },
    telegramEditSender: async (_chatId, messageId, text) => {
      progressEdits.push({ messageId, text });
      return true;
    },
    telegramTypingSender: async () => {
      typingCalls += 1;
    ...
    agentResponder: async (_input, callbacks) => {
      ...
      return "Here is the full final answer.";
    },
  });
  ...
  try {
    ...
    const result = await runtime.handleTelegramWebhook({
      message: {
        text: "Please review the literature and summarize it.",
        chat: { id: "10001" },
        from: { username: "alice" },
      },
    });
    ...
    assert.equal(result.ok, true);
    assert.equal(result.action, "agent_response");
    assert.equal(result.response, "Here is the full final answer.");
    assert.deepEqual(sentTexts, ["Here is the full final answer."]);
    assert.equal(progressMessages.length, 1);
    assert.equal(progressMessages[0].includes("Working on it"), true);
    assert.equal(progressMessages[0].includes("1. Planning approach"), true);
    assert.equal(progressEdits.some((entry) => entry.text.includes("2. Searching papers")), true);
    assert.equal(progressEdits.some((entry) => entry.text.includes("3. Downloading 2 papers")), true);
    assert.equal(progressEdits.some((entry) => entry.text.includes("   - Paper A")), true);
    assert.equal(progressEdits[progressEdits.length - 1]?.text.startsWith("Completed"), true);
    assert.equal(typingCalls > 0, true);
    ...
    assert.equal(entries[1].content, "Here is the full final answer.");
    assert.equal(entries[1].content.includes("@telegram-progress"), false);
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
});
