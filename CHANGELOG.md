...
- Studio chat now renders image attachments inline inside the chat turn through a shared `AttachmentsRow` helper: image-kind files appear as clickable, lazy-loaded thumbnails (`max-h-72 max-w-xs`, rounded with hover shadow) that open the full preview in the right rail, while non-image attachments keep using the existing chip layout. Works the same for user uploads (inside the orange `you` bubble) and agent-returned files (under the assistant Markdown).
- Studio chat transcript gained a top-center "View full session" pill button (lucide `ArrowUpToLineIcon`, secondary variant, rounded full) that smooth-scrolls back to the start of the session, and the composer + inline live-status panel now auto-hide when the user scrolls more than ~80px above the bottom and reappear once they scroll back near the latest message. Auto-scroll on new messages also only triggers when the user is already near the bottom, so a streaming reply never yanks them away from history.
...
- Studio chat now blends into the page background: the central transcript `<section>` no longer draws a border, and the inner `ScrollArea` inside the right-rail "Returned files" list also drops its border so the file list reads as a flat column.
