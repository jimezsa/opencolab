...
- Fixed `spawn ENAMETOOLONG` on the `pi` runtime (xAI, OpenRouter, Kimi), which broke routed runs outright on Windows once an agent had accumulated enough history. `PI_WORKSPACE_ARGS` passed the entire pi system prompt as an argv value (`--append-system-prompt {system_prompt}`) plus the user turn as a trailing positional. That prompt is assembled by `buildPiSystemPromptForInput` from the agent md files, `BUILTIN_TOOLS_CONTEXT`, the skills lists, `MEMORY.md`, the previous-day summary, and the last 8 working-memory turns, so it grows with use until the command line crosses the OS cap — 32 KiB on Windows (`ENAMETOOLONG`), roughly 128 KiB per argument on Linux (`E2BIG`). This is the same failure `039401d` fixed for the `claude` and `gemini` runtimes by moving the prompt to stdin; the pi arg set predates that change and never got it. The defaults now keep both halves out of argv: the system prompt is staged in a temp file under the agent's `.opencolab-provider-output/` and passed as `--append-system-prompt <path>` (pi's `resolvePromptInput` reads file contents when the value names an existing file), and the user turn is piped over stdin (pi's `readPipedStdin`/`buildInitialMessage` prepend it to the initial message). `provider-agent.ts` resolves a new `{system_prompt_file}` token, owns that file's lifecycle including cleanup on error, timeout, and abort, and now picks the stdin payload explicitly — the user turn alone when the system prompt travels by file, so the agent context is not duplicated across both channels. Both previously-shipped pi arg sets (the current one and the older pre-`--mode json` one) are registered as `migratableCliDefaults` on all three providers, so a stored `opencolab.json` is upgraded on load rather than left on the broken shape; a config hand-edited away from either signature will not match and should be re-seeded with `opencolab setup model`. The `ENAMETOOLONG` remediation message is now runtime-aware instead of telling pi users to remove a `{prompt}` token their args never had. `buildProviderInvocationArgs` still inserts `pi --thinking <level>` ahead of a positional user message when a custom arg set has one, and simply appends it for the new defaults. Added an end-to-end runtime test driving `ProviderAgent` with a stand-in CLI that asserts stdin holds exactly the user turn, the staged file holds the system prompt, argv stays far below the Windows cap, and the temp file is removed afterward, plus migration tests for both legacy arg sets across all three providers. Recorded the general "no prompt text in argv" requirement and the pi file/stdin contract in `docs/spec.md`, which the earlier claude-side fix never wrote down.
...
- Added `deepseek/deepseek-v4-pro` to the OpenRouter model options offered during `opencolab ignite`, listed last in the curated list. OpenRouter runs on the `pi` runtime, which has no reasoning-effort wiring, so no `provider.ts` capability entry was added and the OpenRouter provider default stays `openai/gpt-5.5`. Recorded the new option in the `ignite` curated-model-options requirement in `docs/spec.md`.
...
- Simplified Telegram pairing in `opencolab ignite` to a one-step handshake. After the bot token is set, onboarding now waits for the user to send their bot any message and auto-detects the chat id from that message, then marks the chat paired — the manual "Telegram chat id" and "pairing code" prompts are gone. Added `waitForTelegramHandshake` and `fetchTelegramBotUsername` to `telegram-poller.ts` (drains pending updates first so a stale message can't auto-pair, shows a `t.me/<bot>` link via `getMe`, and acknowledges the chat once paired), a `markTelegramPaired` runtime method, and an optional `waitForTelegramHandshake` ignite dependency. On timeout the flow offers retry, manual chat-id entry (the previous chat-id + pairing-code flow, still available as a fallback), or skip; the `opencolab setup telegram pair start|complete` commands are unchanged. Added handshake tests for the poller and ignite. See `docs/telegram_pair_handshake_spec.md`.
...
- Expanded the OpenAI and Anthropic model options offered during `opencolab ignite`. Added the OpenAI models `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna` (listed ahead of `gpt-5.5`) and the Anthropic model `claude-opus-5` (listed ahead of `claude-opus-4-8`) to the onboarding model list in `ignite.ts`. Added matching entries to the `openai` and `anthropic` reasoning-effort capability maps in `provider.ts` so the new models expose reasoning-effort selection during onboarding: the three `gpt-5.6` models support `low|medium|high|xhigh|max` (default `high`), and `claude-opus-5` mirrors `claude-opus-4-8` with `low|medium|high|xhigh|max` (default `high`). Existing model entries and provider defaults are left unchanged.
...
- Added the shared `browser-use` skill at `projects/SKILLS/browser-use/SKILL.md` for automated browser control via the `browser-use` CLI backed by Playwright Chromium. Covers headed and headless sessions, named persistent sessions, optional `--profile` cookie reuse for authenticated sites, and primitives for open, state, click, input, keys, select, scroll, back, screenshot, extract, and close. The shared `BUILTIN_TOOLS.md` template now advertises `browser-use` under Selected Shared Skills so every agent prompt knows when to reach for real browser navigation (portal logins, ATS/job application walkthroughs, form filling, dashboard clickthroughs, rendered-page screenshots, and JavaScript-only content extraction).
- Added the `frontend-design` shared skill to `skills-lock.json` (sourced from `anthropics/skills`), alongside the existing `shadcn` skill entry, so `opencolab` installs pull in the upstream frontend-design guidance for Studio and other frontend work.
- Added an agent-driven install prompt at `docs/install_prompt/install_prompt.md`. The prompt walks any AI agent (Claude Code, ChatGPT desktop, Cursor, etc.) through an end-to-end OpenColab install: Node 20+ / Claude Code CLI preflight, `npm install -g opencolab`, project + agent creation, Anthropic OAuth wiring for `claude-opus-4-7` with reasoning effort `xhigh`, human-guided BotFather bot creation, chat-id lookup via `@userinfobot`, `opencolab setup telegram` wiring, `pair start` / `pair complete` handshake, `opencolab gateway start --port 4646`, and a final smoke test from Telegram. The prompt is OAuth-only (no `ANTHROPIC_API_KEY`) and explicitly refuses `sudo npm install -g`.
