...
The skill should launch jobs in detached mode with `--wait false` and return the `run_id` promptly. Before the agent reports on a launched or monitored run, it should refresh the run with `opencolab gpu job status --run-id <id>` so the latest remote logs are downloaded locally, then review `bootstrap`, `stdout`, `stderr`, and `poller`. It can use `opencolab gpu job exec --run-id <id> --command "<remote command>"` for bounded direct Pod inspection when SSH-backed access is needed. If the user explicitly wants to use a manually created Runpod Pod, or managed provisioning is blocked and the user approves a workaround, the skill may fall back to direct SSH against that Pod, but it should describe that as a manual path outside the normal OpenColab `run_id` lifecycle rather than pretending `gpu job exec` works with a raw `pod_id`. For default or curated target creation, it should prefer the single `NVIDIA A100 80GB PCIe` GPU with `--auto-stop-policy keep_warm`, and if a finished run leaves the Pod warm it should ask whether to keep it running or cancel it.
