...
export function buildPiSystemPromptForInput(
  ...
  return [
    ...
    "OpenColab enables progress updates by default during provider runs. When OPENCOLAB_PROGRESS_FILE is set in the shell environment and the task is long-running, append concise one-line JSON progress events to that file when useful instead of staying silent until the end.",
    "",
    piContext,
    "",
    BUILTIN_TOOLS_CONTEXT,
