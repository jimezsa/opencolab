...
- `browser-use`
  Description: Automated browser control through the `browser-use` CLI backed by Playwright Chromium, with headed or headless sessions, named persistent sessions, optional `--profile` cookie reuse for authenticated sites, and primitives for open, state, click, input, keys, select, scroll, back, screenshot, extract, and close.
  When to use: whenever a task needs real browser navigation, such as logging into a portal, walking through an ATS or job application, filling forms, clicking through dashboards, taking screenshots of a rendered page, or extracting content that only exists after JavaScript runs. Activate the configured Python virtual environment first, set `DISPLAY` on Linux when running headed, prefer named `--session` reuse with `--profile` for sites requiring login, log in by hand once in the visible window rather than typing passwords through the CLI, and close stale sessions with `browser-use close` or `browser-use --session <name> close` when things misbehave.
