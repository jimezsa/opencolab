...
- The shared `runpod-job` skill is the canonical AI-facing path for operating Runpod-backed GPU servers and bounded remote GPU jobs through the OpenColab CLI rather than raw Runpod APIs. It should use `opencolab gpu server availability --server-id <id>` when current datacenter or GPU stock needs to be checked before launch, pay attention to warnings about Pod-API-incompatible datacenters or known storage-provisioning failures, and use `opencolab gpu job exec --run-id <id> --command "<remote command>"` for bounded direct Pod inspection when SSH-backed access is needed. It must launch jobs in detached mode with `--wait false`, return the `run_id` promptly, refresh the run with `opencolab gpu job status --run-id <run_id>` before reporting on it so the latest remote logs are downloaded locally, review `bootstrap`, `stdout`, `stderr`, and `poller` when summarizing a run, prefer a single `NVIDIA A100 80GB PCIe` GPU with `keep_warm` for curated/default targets, and ask the user whether to keep a finished warm Pod running or cancel it. If the user explicitly wants to use a manually created Runpod Pod, or managed provisioning is blocked and the user approves a workaround, the skill may use direct SSH as a clearly labeled manual fallback, but it must not claim that a raw `pod_id` is an OpenColab `run_id` or that `opencolab gpu job exec` works directly against a raw `pod_id`. Curated/default Runpod targets should use `pytorch-cu12`, and failed or degraded runs should be surfaced clearly with a proposed next action.
