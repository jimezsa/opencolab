...
- The shared `fast-search`, `pro-search`, and `deep-search` skills must keep their `findings.md` formats stable while returning concise, friendly channel-agnostic summaries and, when appropriate, returning `findings.md` through the active channel's file-delivery mechanism.
