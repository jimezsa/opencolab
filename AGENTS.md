...
- The shared `runpod-job` skill is the canonical AI-facing path for operating Runpod-backed GPU servers and bounded remote GPU jobs through the OpenColab CLI rather than raw Runpod APIs. For longer jobs it should prefer detached launch with `--wait false`, return the `run_id` promptly, and inspect status or logs later only when the user explicitly asks about the run or asks to monitor it. Curated/default Runpod targets should use `pytorch-cu12`, and failed or degraded runs should be surfaced clearly with a proposed next action.
