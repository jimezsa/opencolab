...
- The shared `block-diagram` skill defaults to compact layouts with unlabeled arrows, supports optional LaTeX equation blocks when they materially clarify a model or pipeline, may consult its optional ML/LLM architecture template for neural-network, transformer, training, or quantization diagrams, and only uses edge labels when they carry concrete meaning such as a protocol or payload, not generic `input` or `output` text.
- The shared `fast-research`, `pro-research`, and `deep-research` skills must keep their `findings.md` formats stable while storing each distinct research topic in its own dated, topic-slugged `research/<YYYY-MM-DD>-<topic-slug>/` folder, maintaining root `research/INDEX.md` and per-run `RUN.md` metadata, producing a companion literature-map block diagram through `block-diagram`, returning concise, friendly channel-agnostic summaries, and, when appropriate, returning `findings.md` plus a PNG-first diagram through the active channel's file-delivery mechanism, with SVG fallback if PNG rendering is unavailable.
- Research output contract (consumed by OpenColab Studio's research browser, see `docs/research_browser_spec.md`):
  - **Location resolution.** If `OPENCOLAB_ACTIVE_AGENT` is set, write under `projects/<project>/AGENTS/<agent>/research/`. Otherwise write under `projects/<project>/research/`. All artifacts must stay inside the chosen run folder — never write to `~/Downloads` or paths outside the run.
  - **Folder naming.** `<YYYY-MM-DD>-<topic-slug>` where the slug is lowercased, ASCII-only, hyphen-separated, and ≤ 60 chars. Collisions append `-2`, `-3`, ….
  - **`RUN.md` frontmatter.** Each run folder must contain a `RUN.md` whose first block is YAML frontmatter delimited by `---` lines. Required keys: `skill`, `topic`, `status` (one of `running`, `complete`, `failed`, `abandoned`), `created` (ISO 8601). Optional keys: `question`, `updated`, `corpus.papers`, `corpus.summaries`, `corpus.diagrams`, `deliverables` (a flat YAML list of relative paths). Free-form notes belong below the frontmatter.
  - **`INDEX.md` columns.** A markdown table with one row per run, columns in fixed order: `Folder | Skill | Topic | Status | Created | Updated | Corpus | Deliverables | Notes`.
  - **Artifact placement.** PDFs in `pdf/<safe_id>.pdf`; colocated `pdf/<safe_id>.md` per-paper summaries; diagrams in `diagrams/*.{png,svg,d2}` (PNG preferred); metadata in `meta/`; search results in `search/`; PageIndex output in `pageindex/` (`trees/`, `manifest.json`, `answers/`).
- The shared `pageindex-grounded` skill is the canonical local-first path for grounded follow-up QA over already-downloaded papers. It must keep selection bounded, prefer the active topic-scoped research folder selected from `research/INDEX.md`, cache PageIndex trees under that run folder's `pageindex/`, use `GEMINI_API_KEY` for the local PageIndex runner, and answer with exact paper or page references plus explicit limitations when coverage is partial.
- The shared `pdf-figure-extract` skill is the canonical local-first path for extracting and returning figures from already-downloaded papers. It must use PyMuPDF, prefer the active topic-scoped research folder selected from `research/INDEX.md`, work even when PageIndex artifacts are missing, optionally reuse that run folder's `pageindex/` artifacts to narrow candidate pages, and require multimodal verification of shortlisted figure candidates when the active provider supports local image inspection.
- The shared `latex-paper-writer` skill is the canonical path for creating, editing, versioning, compiling, and returning scientific LaTeX papers, reports, and research-derived PDF summaries. It must keep each paper workspace under Git version control, use venue-aware templates without claiming official compliance unless verified, compile with `latexmk` when available while surfacing install guidance when missing, generate experiment-result tables, reuse `deep-research`/`pro-research`/`fast-research` findings and `pageindex-grounded` evidence, use `pdf-figure-extract` and `block-diagram` for visuals when appropriate, and return final PDFs through the active channel's file-delivery mechanism when requested.
- The shared `autoresearch` skill is the canonical path for iterative keep/discard experiment loops over one explicitly configured repo. It is a normal shared skill available to every agent, but the built-in `autoresearch` agent is the default owner for sustained experiment-loop work. It must require explicit `repo_path`, `editable_file_path`, `run_command`, and `metric_rule`, treat non-zero exit or missing metric as failure by default, use a dedicated disposable branch or worktree for keep/discard iteration, must not assume `train.py` or `uv run train.py`, and must include a progress-graph helper for key-metric-over-experiment plots with green kept experiments, gray discarded or non-kept experiments, short kept-experiment labels, and a running-best line.
...
- The seeded `AGENTS.md` contract must explain that OpenColab owns Telegram live status for routed runs, derives it from runtime events instead of an agent-written progress file, expects agents to avoid low-signal step-by-step chatter, and may surface a bounded recent tool-activity list in a persistent Telegram live-status message that remains visible after the final answer.
